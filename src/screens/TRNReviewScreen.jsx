...
function EditRow({ question, trnAnswers, currentValue, onSave, onCancel }) {
  const [text, setText] = useState(currentValue || '')
  const options = question.options || (question.getOptions ? question.getOptions(trnAnswers) : null)
...
function SectionCard({ title, color, entries, editState, onEdit, onSaveEdit, trnAnswers }) {
    ...
    <div className="mb-5">
      ...
      <div className="rounded-2xl overflow-hidden" style={{ border: '1.5px solid #f3f4f6' }}>
        {entries.map(({ label, value, questionId }, i) => {
          const q = questionId ? TRN_QUESTIONS[questionId] : null
...
export default function TRNReviewScreen({ trnAnswers, onBack, onFormReady, onEditAnswer }) {
