...
const COMPLETE_MSG =
  'You have completed the first section! Great job. Take a breath — you are doing really well.'

// Fixed confetti particles (useMemo makes them stable across re-renders)
  ...
  return useMemo(
    () =>
      Array.from({ length: 45 }, (_, i) => ({
        id: i,
        x: 2 + (i / 45) * 96,           // spread 2%–98% across width
        color: COLORS[i % COLORS.length],
        delay: (i % 9) * 0.2,            // stagger 0–1.8 s
        duration: 2.2 + (i % 5) * 0.35, // vary 2.2–3.6 s
        size: 6 + (i % 6) * 2.5,        // 6–17.5 px
        isCircle: i % 3 === 0,
        drift: ((i % 7) - 3) * 30,       // horizontal drift –90..90 px
      })),
    [],
  )
}

export default function SectionACompleteScreen({ onContinue }) {
    ...
    <div className="flex flex-col min-h-screen bg-white overflow-hidden relative">
      ...
      <div className="relative z-10 flex-1 flex flex-col items-center justify-center px-6 py-10">
        ...
        <p className="text-gray-500 text-lg text-center leading-relaxed mb-10 px-2">
          you are doing <strong className="text-black">really well</strong>.
        </p>
        ...
        >
          ...
          <div>
            <p className="text-black font-bold text-sm">Section A — Personal Information</p>
            <p className="text-gray-400 text-xs mt-0.5">All 18 questions answered ✓</p>
          </div>
        </div>
